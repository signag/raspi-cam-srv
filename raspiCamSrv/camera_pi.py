...
class Camera(BaseCamera):
    ...
    def __init__(self):
        ...
        if Camera.cam is None:
            logger.debug("Thread %s: Camera.__init__: Camera instantiated: %s", get_ident(), activeCam)
            Camera.cam = Picamera2(activeCam)
            Camera.camNum = activeCam
        else:
            if activeCam != Camera.camNum:
                logger.debug("Thread %s: Camera.__init__: About to switch camera from %s to %s", get_ident(), Camera.camNum, activeCam)
                Camera.stopCameraSystem()
                Camera.cam = Picamera2(activeCam)
                Camera.camNum = activeCam
                logger.debug("Thread %s: Camera.__init__: Switch camera to %s successful", get_ident(), activeCam)
                # Force refresh of camera properties
                CameraCfg().cameraProperties.model=None
                CameraCfg().sensorModes = []
                CameraCfg().rawFormats = []
                logger.debug("Thread %s: Camera.__init__: Camera-specific configs were reset", get_ident())
            else:
